...
const Dashboard = {
  ...
  async afterRender() {
    ...
    const imageWeather = document.getElementById('img-icon-weather');
    ...
    try {
      ...
      imageWeather.src = weather.current.condition.icon;
      weatherName.innerText = weather.current.condition.text;
      Swal.close();
    } catch (error) {
      Swal.fire({
        icon: 'error',
